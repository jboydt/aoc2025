pub fn solve(lines: &Vec<String>) -> (i64, i64) {
    ...
    for battery_bank in lines {
        solution_one += get_joltage_two(&battery_bank, 2, 0);
        solution_two += get_joltage_two(&battery_bank, 12, 0);
    }
...
fn get_joltage_two(battery_str: &str, needed: usize, sum: i64) -> i64 {
    let mut battery_bank = String::from(battery_str);
    ...
    let max_scaled = (10 as i64).pow(needed as u32 - 1) * max as i64;
    if needed == 1 {
        return sum + max;
    } else {
        return sum + max_scaled + get_joltage_two(&battery_str[max_index + 1..], needed - 1, sum);
    }
}
